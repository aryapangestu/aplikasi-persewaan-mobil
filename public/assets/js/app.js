...
(function($) {
    ...
    $(document).ready(function() {
        const rangePicker = $("#disabled-range-picker");
        const hargaSewaElement = $("#harga-sewa");
        const modelSewaElement = $("#model-sewa");

        $(".btn-modal-trigger").on("click", function() {
            const carId = $(this).data("car-id");

            const mobilModel = $(this).data("car-model");
            modelSewaElement.text("Sewa Mobil " + mobilModel);

            const tarifHarian = parseFloat($(this).data("rental-rate-per-day")) || 0;
            ...
            rangePicker.flatpickr({
                ...
                onChange: function(selectedDates, dateStr, instance) {
                    if (selectedDates.length === 2) {
                        ...
                        // Tambahkan 1 hari ke endDate
                        const tempEndDate = new Date(endDate);
                        tempEndDate.setDate(tempEndDate.getDate() + 1);
                        endDate = tempEndDate.toISOString().split('T')[0];

                        const hargaSewa = hitungHargaSewa(startDate, endDate, tarifHarian);
                        hargaSewaElement.text(formatMataUang(hargaSewa));
                    } else {
                        startDate = null;
                        endDate = null;
                        hargaSewaElement.text("Rp 0");
                    }
                }
            });

            const sewaButton = $("#sewa-button");

            sewaButton.off("click"); // Remove previous click event handler
            sewaButton.on("click", function() {
                if (startDate && endDate) {
                    const sewaLink = `/sewa/${carId}?start_date=${startDate}&end_date=${endDate}`;
                    window.location.href = sewaLink; // Redirect to the sewaLink
                } else {
                    alert("Please select both start and end dates for rental");
                }
            });
        });

        function hitungHargaSewa(startDate, endDate, tarifHarian) {
            const durasiSewa = Math.ceil((new Date(endDate) - new Date(startDate)) / (1000 * 60 * 60 * 24));
            const hargaSewa = durasiSewa * tarifHarian;
            return hargaSewa;
        }
    });
